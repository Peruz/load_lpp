...
Load\_LPP could serve as a reference because the language ecosystem for data management is relatively young and there are not many complete data pipelines.

![Example of usage in edge devices, as from previous scientific studies [@peruzzo2024]. \label{fig1}](figure.png)

# Statement of need
Weighing systems are critical and safety components in many fields, from drug dosing to large systems in farming, construction, and mining.
...
The temporal dynamics of such issues give variable levels of signal (i.e., load changes) and noise.
...
Data use timezone-aware datetime and also handles clock variations over the year (RFC 3339 - ISO 8601).
...
The processing functionalities offer common processing steps, as well as more advanced and specific time-series operations.
...
The second processing step removes periods specified by the user (maintenance, etc.), these are stored in a plain text file.
...
Flexible smoothing solutions are available to smooth and gap-fill the data.
A weighted moving average is defined with user-defined width and weight distribution (between a central-maximum value and side-minimum weights).
A maximum missing weight is also defined so that the central value being interpolated is left to NaN when too many measurements, and thus weight, are missing within the window.
...
strong signal (significant-fast load changes) favors shorter windows to avoid excessive smoothing; however, this is balanced by the relative relevance of the noise, which favors longer windows.

The visualization executable provides a convenient and interactive visualization of the data, reading either the raw or processed data.
